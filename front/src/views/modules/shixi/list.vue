        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '实习结果' : ''">
                        <el-select v-model="searchForm.shixiJieguoTypes" placeholder="请选择实习结果">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in shixiJieguoTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('shixi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/biyeshengshixiyujiuye/upload/shixiMuBan.xls"
                        >批量导入实习信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shixi','导入导出')"
                                ...
                                :on-success="shixiUploadSuccess"
                                :on-error="shixiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('shixi','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入实习信息数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="实习类型">
                        <template slot-scope="scope">
                            {{scope.row.shixiValue}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixiKaishiTime"
                                   header-align="center"
                                   label="实习开始时间">
                        <template slot-scope="scope">
                            {{scope.row.shixiKaishiTime}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixiJieshuTime"
                                   header-align="center"
                                   label="实习结束时间">
                        <template slot-scope="scope">
                            {{scope.row.shixiJieshuTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="shixiJieguoTypes"
                                      header-align="center"
                                      label="实习结果">
                        <template slot-scope="scope">
                            {{scope.row.shixiJieguoValue}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shixiGangweiName"
                                   header-align="center"
                                   label="实习岗位">
                        <template slot-scope="scope">
                            {{scope.row.shixiGangweiName}}
                        </template>
                    </el-table-column>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'shixi',//当前表表名,
                        sumColum : 'shixi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'shixi_name',//分组字符串字段
                        // types : 'shixi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.shixiJieguoTypes!= '' && this.searchForm.shixiJieguoTypes!= undefined) {
                    params['shixiJieguoTypes'] = this.searchForm.shixiJieguoTypes
                }
                                
                params['shixiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=shixi_jieguo_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.shixiJieguoTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
